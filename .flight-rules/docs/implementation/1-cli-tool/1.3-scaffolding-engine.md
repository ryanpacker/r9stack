...
```
1. TanStack Start  →  Base React project with routing
2. shadcn/ui       →  Tailwind CSS + component library
3. Convex          →  Backend and database layer
4. WorkOS          →  Authentication integration (templates)
```
...
**⚠️ Validation Required: TanStack Start CLI**

Before implementation, validate which flags the TanStack Start CLI supports for non-interactive mode:
- Test `npm create @tanstack/start@latest` with `--yes`, `--name`, `--template` flags
- Document which prompts can be bypassed with flags
- r9stack's interactive prompts should collect any info needed to pass through
- **Fallback:** If non-interactive mode isn't well-supported, bundle a known-good project structure as templates
