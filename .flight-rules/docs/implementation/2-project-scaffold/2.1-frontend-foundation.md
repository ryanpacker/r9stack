...
src/
├── components/
│   ├── ui/                     # shadcn components (from add-on)
│   │   └── button.tsx
│   ├── AppShell.tsx            # Main layout wrapper (r9stack)
│   ├── AuthProvider.tsx        # Auth context provider (r9stack)
│   ├── ConvexClientProvider.tsx # SSR-safe Convex wrapper (r9stack)
│   ├── NavGroup.tsx            # Collapsible nav section (r9stack)
│   ├── NavItem.tsx             # Navigation link (r9stack)
│   ├── Sidebar.tsx             # Collapsible sidebar (r9stack)
│   └── UserMenu.tsx            # Auth-aware user dropdown (r9stack)
├── lib/
│   ├── auth.ts                 # Auth types (r9stack)
│   ├── auth-client.ts          # Client-side auth context (r9stack)
│   ├── auth-server.ts          # Server-side auth with iron-session (r9stack)
│   └── utils.ts                # cn() helper (from shadcn add-on)
├── routes/
│   ├── __root.tsx              # Root layout with providers
│   ├── index.tsx               # Public landing page
│   ├── auth/
│   │   ├── callback.tsx        # OAuth callback handler
│   │   ├── sign-in.tsx         # Redirects to WorkOS AuthKit
│   │   └── sign-out.tsx        # Clears session
│   └── app/
│       ├── route.tsx           # Auth guard + AppShell wrapper
│       ├── index.tsx           # Authenticated home
│       └── demo/
│           └── convex.messages.tsx  # Demo Convex page
├── router.tsx                  # Router configuration
├── routeTree.gen.ts            # Auto-generated route tree
└── styles.css                  # Tailwind + shadcn styles
```

**Status:** ✅ Complete (structure validated in sandbox)
...
**Status:** ✅ Complete (validated in sandbox)
...
> **Note:** If the shadcn add-on doesn't install components, the CLI should run the add command.

**Status:** 🔵 Planned (depends on add-on gap analysis)
