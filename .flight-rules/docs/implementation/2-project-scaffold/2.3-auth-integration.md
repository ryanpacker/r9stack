...
> **Dependency Note:** This analysis is part of the broader gap analysis task (1.3.1b). The findings will determine which auth files are included in the r9stack payload directory vs relying on add-on output. The patterns documented in 2.3.1–2.3.5 are validated in the sandbox and represent the target state regardless of add-on output.

### To Investigate

When running with `--add-ons workos`:
- Does it create auth route files (`/auth/sign-in`, `/auth/callback`, `/auth/sign-out`)?
- Does it configure iron-session or another session library?
- Does it create an AuthProvider component?
- What session management approach does it use?
- Does it create `lib/auth*.ts` files?

### Expected Gaps (Payload Templates Likely Needed)

Based on the validated target, r9stack likely needs to provide these as payload templates:

| File | Reason |
|------|--------|
| `lib/auth.ts` | Type definitions may not match our shape |
| `lib/auth-client.ts` | AuthContext with our specific interface |
| `lib/auth-server.ts` | iron-session integration with TanStack Start server functions |
| `components/AuthProvider.tsx` | Provider component matching our context |
| `routes/auth/*.tsx` | Auth routes if not provided by add-on |
| `routes/app/route.tsx` | Protected route wrapper (unlikely from add-on) |

### Outcome

After gap analysis, update:
1. The payload directory (1.3.2) with confirmed needed files
2. This section with documented findings

**Status:** 🔵 Planned (blocking dependency: 1.3.1b)
